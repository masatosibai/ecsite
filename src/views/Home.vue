...
export default {
  ...
  async created() {
    const data = await axios.get(process.env.VUE_APP_API_PRODUCTION + "/shops");
    const area = [];
    const genre = [];
    ...
    // console.log(process.env.VUE_APP_API_PRODUCTION);
  },
  mounted() {
  ...
  methods: {
    ...
    favoriteStore(storeID) {
      if (this.$store.state.auth) {
        axios
          .post(process.env.VUE_APP_API_PRODUCTION + "/likes", {
